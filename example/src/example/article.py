...
def parse_article(context: object, data: dict, article: Article) -> None:
    with context.http.rehash(data) as result:
        if result.html is not None:
            ...
            data["properties"] = {
                "title": (
                    result.html.xpath(properties["title"])
                    if properties.get("title")
                    else getattr(article, "title", None)
                ),
                "description": (
                    result.html.xpath(properties.get("description"))
                    if properties.get("description")
                    else getattr(article, "description", None)
                ),
                "author": (
                    result.html.xpath(properties.get("author"))
                    if properties.get("author")
                    else getattr(article, "authors", None)
                ),
                "publishedAt": (
                    result.html.xpath(properties.get("publishedAt"))
                    if properties.get("publishedAt")
                    else getattr(article, "publish_date", None)
                ),
                "bodyText": (
                    result.html.xpath(properties.get("bodyText"))
